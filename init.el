...
;; if using a different folder for your config set this to that folder
;;(setq user-emacs-directory "~/.emacs.42.d/")

;; some packages may fail checking signature because emacs used on 42 is vey old
...
;; on 42 some packges do not install because it is a version too old, using versioned versions
;; that lives on folder ./versioned-oldpackages
(let ((package-dir (expand-file-name "versioned-oldpackages/" user-emacs-directory)))
  (add-to-list 'load-path package-dir)
  (dolist (pkg-folder-to-add (directory-files package-dir t "\\w+"))
    (when (file-directory-p pkg-folder-to-add)
      (add-to-list 'load-path pkg-folder-to-add))))
...
(use-package emacs
  ...
  (ido-mode t)
  ;; memorize layout history (C-c <- and C-c -> to go back and forward)
  (winner-mode 1)
  ...
  ;; easy file move/copy on dired with side by side buffers
  (setq dired-dwim-target t)
  ;; save layout during session
  (winner-mode 1)
  ;; recent opened files history
  (recentf-mode 1)
  ;; auto refresh tags table
  (setq tags-revert-without-query t)

  ;; custom folders for emacs "temporary/versioning" files
  (let ((autosaves-dir (expand-file-name "auto-saves/" user-emacs-directory))
    ...
    (unless (file-exists-p backups-dir)
      (make-directory backups-dir nil))
...
;; Add Allman style to the list of C styles
(c-add-style "allman" allman-style)

(defun 42-c-mode-hook-fun()
  ...
  (setq-local c-default-style "allman")
  (setq-local tab-stop-list '(4 8 12 16 20 24 28 32 36 40 44 48 52 56 60
	  		  				 64 68 72 76 80 84 88 92 96 100 104 108 112 116 120)))

(add-hook 'c-mode-hook #'42-c-mode-hook-fun)
(add-hook 'c++-mode-hook #'42-c-mode-hook-fun)

;; color output on compilation mode
...
;; emacs on 42 is too old using versioned version of deadgrep
(require 'deadgrep)
...
;; ;; (set-face-attribute 'hl-line nil :foreground nil :background "#CCCCCC")
...
(use-package zenburn-theme
  ...
  (set-face-attribute 'region nil :background "#FFFF30" :foreground "#000099")
...
;; ;; (set-face-attribute 'hl-line nil :foreground nil :background "#000000")
;; ;; (setq-default default-frame-alist '((background-color . "#00000000")))
;; ;; ;;Theme
...
;; 42 emacs is to old to install magit through elpa
;; using a versioned magit version
(require 'magit)
;; ;; git interface 
;; ;; (use-package magit
;; ;;   :ensure t) 

;; ;; ssh integration (edit remote files, run remote shell...) 
;; (use-package tramp
;;   :ensure t)
...
(use-package eglot
  ...
  (push
   '(c-mode
     "/home/maurodri/clangd-linux-16.0.2/clangd_16.0.2/bin/clangd"
